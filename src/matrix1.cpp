...
void matrix1::size(uint32_t n) { this->n = n; }

uint32_t matrix1::idx(uint32_t n) const { return n; }

uint32_t matrix1::size() const { return n; };
...
matrix1 matrix1::zeros(uint32_t n) {
  matrix1 M;
  M.size(n);
...
matrix1 matrix1::ones(uint32_t n) {
  matrix1 M;
  M.size(n);
  M.mat = new float[M.size()];
...
matrix1 matrix1::random(uint32_t n) {
...
matrix1 operator*(matrix1 const &M, matrix1 const &C) {
  std::array<uint32_t, 1> sz_A{M.n};
  std::array<uint32_t, 1> sz_B{M.n};
